...
const operations = [
  [Smartphone, 'CARREGAMENTOS', 'topup'],
  [ArrowRight, 'TRANSFERÊNCIAS EXPRESS', 'express'],
  [Banknote, 'LEVANTAMENTOS SEM CARTÃO', 'services'],
  [Receipt, 'PAGAMENTOS', 'pay'],
  [ArrowLeft, 'TRANSFERÊNCIAS', 'channels'],
  [QrCode, 'COMPRAS', 'pay'],
  [Banknote, 'CONSULTAS', 'balance'],
  [ShieldCheck, 'OPERAÇÕES POR AUTORIZAR', 'history'],
  [CreditCard, 'DÉBITOS DIRECTOS', 'pay'],
] as const

function Brand({white=false,src}:{white?:boolean;src?:string}){return <img className="brand-logo" src={src||(white?'/assets/mcx-logo-header.png':'/assets/mcx-logo-reference.jpg')} alt="MULTICAIXA Express"/>}
function Splash({next}:{next:()=>void}){useEffect(()=>{const timer=setTimeout(next,1600);return()=>clearTimeout(timer)},[next]);return <main className="splash-screen"><Brand/></main>}
function Header({back,menu,logoSrc}:{back?:()=>void;menu?:()=>void;logoSrc?:string}){return <header className="app-header">{back?<button className="header-button" onClick={back} aria-label="Voltar"><ChevronLeft size={34}/></button>:<button className="header-button" onClick={menu} aria-label="Abrir menu"><Menu size={30}/></button>}<Brand white src={logoSrc}/><span className="header-spacer"/></header>}
function Support(){return <section className="support"><h2>APOIO AO CLIENTE</h2><strong>Linha de Atendimento 24H</strong><p><Phone size={18}/><u>(+244) 222 641 840</u> | <u>923 168 840</u></p><p><Mail size={18}/><u>suporte.mcxexpress@emis.co.ao</u></p><h3><Info size={20}/>Precisa de ajuda?</h3></section>}
function Pin({onDone,title='PIN do serviço MULTICAIXA'}:{onDone:()=>void;title?:string}){const [pin,setPin]=useState('');const press=(key:string)=>{if(key==='APAGAR')setPin(p=>p.slice(0,-1));else if(pin.length<6){const next=pin+key;setPin(next);if(next.length===6)setTimeout(onDone,150)}};return <main className="phone-shell pin-screen"><div className="pin-top"><Brand/></div><section className="pin-content"><div className="pin-dots">{[0,1,2,3,4,5].map(i=><span key={i} className={i<pin.length?'filled':''}/>)}</div><h1>{title}</h1><div className="keypad">{['1','2','3','4','5','6','7','8','9','APAGAR','0'].map(k=><button key={k} className={k==='APAGAR'?'delete-key':''} onClick={()=>press(k)}>{k}</button>)}</div></section>{title.startsWith('PIN do serviço')&&<Support/>}</main>}
function Card({bpc=false}:{bpc?:boolean}){return <div className={`carousel-card ${bpc?'bpc':'atlantico'}`}><img className="real-bank-card" src={bpc?'/assets/bpc-card-display.png':'/assets/atlantico-card.jpg'} alt={bpc?'Cartão bancário BPC':'Cartão bancário Atlântico'}/></div>}
function Home({go,menu,selectedCard,setSelectedCard}:{go:(s:Screen)=>void;menu:()=>void;selectedCard:number;setSelectedCard:(card:number)=>void}){const card=selectedCard;return <main className="phone-shell"><Header menu={menu}/><section className="home-content"><h1 style={{fontWeight:700}}>CARTÕES</h1><div className="cards-viewport"><div className={`cards-track ${card===1?'show-bpc':''}`}><button className="card-switch" onClick={()=>setSelectedCard(card===0?1:0)}><Card bpc={card===1}/></button></div></div><div className="card-caption">Cartão {card+1}<div className="carousel-dots"><i className={card===0?'active':''} onClick={()=>setSelectedCard(0)}/><i className={card===1?'active':''} onClick={()=>setSelectedCard(1)}/></div></div><div className="operations">{operations.map(([,label,target])=>{const expressInactive=label==='TRANSFERÊNCIAS EXPRESS'&&card===1;return <button key={label} className={expressInactive?'op-inactive':''} aria-disabled={expressInactive} onClick={()=>{if(expressInactive)return;go(target as Screen)}}><img className="grid-icon-image" src={`/icons/${['carregamentos','transferencias-express','levantamentos-sem-cartao','pagamentos','transferencias','compras','consultas','operacoes-por-autorizar','debitos-directos'][operations.findIndex(([,itemLabel]) => itemLabel === label)]}.png`} alt="" aria-hidden="true"/><span>{label}</span></button>})}</div></section></main>}
function Bottom({go,active}:{go:(s:Screen)=>void;active:Screen}){return <nav className="bottom-nav"><button className={active==='home'?'active':''} onClick={()=>go('home')}><CreditCard size={21}/>Início</button><button className={active==='pay'?'active':''} onClick={()=>go('pay')}><Banknote size={21}/>Pagar</button><button className={active==='history'?'active':''} onClick={()=>go('history')}><Receipt size={21}/>Movimentos</button><button className={active==='cards'?'active':''} onClick={()=>go('cards')}><WalletCards size={21}/>Cartões</button><button onClick={()=>go('home')}><Menu size={21}/>Mais</button></nav>}
...
export default function Page(){const [screen,setScreen]=useState<Screen>('splash');const [selectedCard,setSelectedCard]=useState(0);const [drawer,setDrawer]=useState(false);const [draft,setDraft]=useState<Op|null>(null);const [operationKind,setOperationKind]=useState<Screen>('pay');const [items,setItems]=useState<Op[]>([]);const openReceipt=(transaction:Op)=>{setDraft(transaction);setScreen('activity-detail')};const [success,setSuccess]=useState(true);const [now, setNow] = useState('')
  useEffect(() => {
    setNow(new Date().toLocaleString('pt-PT', {
      dateStyle: 'short',
      timeStyle: 'short'
    }))
